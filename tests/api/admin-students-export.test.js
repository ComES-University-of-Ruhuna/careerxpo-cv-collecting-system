...
jest.mock('../../src/models/User', () => ({ __esModule: true, default: { find: jest.fn() } }));
...
describe('admin student export', () => {
  ...
  it('exports every matching student with bid job IDs and company-labelled openings', async () => {
    const response = await GET({ url: 'http://localhost/api/admin/students?export=1&department=COM&sort=name' });
    ...
    expect(studentQuery.sort).toHaveBeenCalledWith({ full_name: 1 });
